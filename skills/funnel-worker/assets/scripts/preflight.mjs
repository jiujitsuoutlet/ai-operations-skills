...
if (!cart) {
  fail('THRIVECART_BUNDLE_URL is empty in wrangler.toml. Paste the real the cart platform link for the the offer.');
...
if (!dbId || dbId === 'REPLACE_WITH_D1_DATABASE_ID') {
  fail('database_id is still the placeholder in wrangler.toml. Run `wrangler d1 create PROJECT_DB` and paste the real id.');
...
// the cart platform wires its own accept and decline URLs when the page is pasted into
...
// the cart platform pages are deliverables too, so their copy gaps are blockers as well.
let tcPages = [];
try {
  tcPages = readdirSync(join(ROOT, 'src', 'cart')).filter((f) => f.endsWith('.html'));
} catch {
  /* directory is optional */
}
for (const page of tcPages) {
  sweep(`thrivecart/${page}`, readFileSync(join(ROOT, 'src', 'cart', page), 'utf8'));
}
...
if (expected.length) {
  pass(`${expected.length} the cart platform URL placeholder${expected.length > 1 ? 's' : ''} left unfilled on purpose (wired inside the cart platform)`);
}

/* 5b. generated the cart platform pages are current */
if (tcPages.length) {
  const css = readFileSync(join(ROOT, 'src', 'pages', '_base.css'), 'utf8');
  const stale = [];
  for (const page of tcPages) {
    const built = readFileSync(join(ROOT, 'src', 'cart', page), 'utf8');
    let template = '';
    try {
      template = readFileSync(join(ROOT, 'src', 'cart', '_src', page), 'utf8');
    } catch {
      stale.push(`${page} has no source in src/cart/_src`);
      continue;
    }
    if (built !== template.replace(/\{\{BASE_CSS\}\}/g, () => css)) stale.push(page);
  }
  if (stale.length) {
    fail(`the cart platform pages are stale, run \`npm run build:tc\`: ${stale.join(', ')}`);
  } else {
    pass(`${tcPages.length} the cart platform pages built and current`);
  }
}
...
const COLLECTIONS = [
  // REPLACE: required section names for your offer
  '[Required section 1]', '[Required section 2]', '[Required section 3]',
];
for (const variant of ['a', 'b']) {
  ...
  if (absent.length) missing.push(`${absent.length} of the required sections (${absent.join(', ')})`);
  if (missing.length) fail(`variant ${variant.toUpperCase()} (${file}) is missing ${missing.join('; ')}`);
  else pass(`variant ${variant.toUpperCase()} renders complete (CTA, price, form, all required sections)`);
}
