...
(function () {
   ...
   * clicks, not money: the cart platform owns the purchase, so the variant has to
   * ride along and come back on the webhook.
   *
   * passthrough[variant] is the cart platform's documented custom-data convention (verify per platform).
  ...
  function remember(email) { try { sessionStorage.setItem('fnl_email', email); } catch (_) {} }
  function recall() { try { return sessionStorage.getItem('fnl_email') || ''; } catch (_) { return ''; } }
